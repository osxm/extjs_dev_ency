...
Ext.define('Skirtle.grid.column.Component', {
    ...
    // Whether or not to pass the other fields from the record to the template
    includeFields: null,
    ...
    addIsLayoutChild: function(child) {
        var me = this,
            fn = me.layoutChildFn || (me.layoutChildFn = function() {
                // Don't treat injected components as children from a layout perspective
                return false;
            });

        // Introduced in 5.1
        if (child.isLayoutChild) {
            child.isLayoutChild = fn
        }
    },
    ...
    createRenderer: function(renderer) {
        ...
        return function(value, p, record) {
            ...
            // Extra fields can severely slow down the template in some cases, so only include them if required
            if (me.includeFields) {
                Ext.apply(data, record.data, record.getAssociatedData());
            }
    ...
    createTemplate: function(tpl) {
        // If we haven't been passed a tpl then we auto-generate one which doesn't need any of the other fields
        if (this.includeFields == null) {
            this.includeFields = !!tpl;
        }
    ...
    getInjectedItems: function(deep) {
        var items = [],
            ids = this.compIds,
            index = 0,
            len = ids.length,
    ...
    getRefItems: function(deep) {
        var items = this.callParent([deep]),
            injected = this.getInjectedItems(deep),
            index = 0,
            len = injected.length;

        for ( ; index < len ; index++) {
            items.push(injected[index])
        }
    ...
    redoScrollbars: function() {
        ...
        if (grid) {
            ...
            if (me.extVersion < 40100) {
                ...
                if (grid.view && grid.view.refreshScroll) {
                    // 5.1
                    grid.view.refreshScroll()
                }
    ...
    resizeAll: function() {
        ...
        me.suspendResizing();
        me.resizeQueue = me.getInjectedItems();
        me.resumeResizing();
    },
